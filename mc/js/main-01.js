(function($){
	...
	MCBM.main = function() {
	  ...
	  function _init() {
	    ...
	    // 3D scene
	    camera = new THREE.Camera( 50, winW/winH, 1, 10000 );
	  ...
	  function _createVoxel(position) {
	    ...
	    if (destGridPos.z <= maxLayers) {
	      ...
	      var materials = _getMaterialsByLayer(destGridPos.z);
	      ...
	      for (side in sharedSides) {
	        ...
	        if ( sharedSides[side] ) { // side is shared
	          ...
	          var updateMaterials = _getMaterialsByLayer(updateGridPos.z);        
	  ...
	  function _getMaterialsByLayer(layer) {
    
	    var materialsIndex = materialIndexes.TOP;
    
	    if (layer >= materialLayerStart.ROCK) {
	      materialsIndex = materialIndexes.ROCK;
	    } else if (layer >= materialLayerStart.DIRT) {
	      materialsIndex = materialIndexes.DIRT;
	    }
    
	    return materialsLookup[materialsIndex];
